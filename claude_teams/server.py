...
def _build_spawn_description(
    ...
    if show_codex:
        backends.append("'codex' (uses ~/.codex/config.toml model setting)")
    if backends:
...
async def app_lifespan(server):
    ...
    if not claude_binary and not opencode_binary and not codex_binary:
        raise FileNotFoundError(
            "No coding agent binary found on PATH. "
            "Install Claude Code ('claude'), OpenCode ('opencode'), or Codex CLI ('codex')."
        )
    ...
    async def on_initialize(self, context, call_next):
        ...
        if native_backend and native_backend not in enabled:
            if (
                ...
                or native_backend == "codex"
                or _lifespan_state.get("opencode_server_url")
            ):
