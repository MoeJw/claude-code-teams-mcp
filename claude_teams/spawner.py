...
def build_codex_spawn_command(
    ...
    """Build the interactive codex command to run in a tmux pane.

    Runs ``codex`` in fully-automatic non-interactive mode so it can start
    immediately without waiting for user input in the terminal.  The initial
    prompt is passed as a positional argument.  The working directory is set
    via ``cd`` before invoking codex so file paths in the prompt resolve
    correctly.

    We deliberately use ``codex exec --full-auto`` (non-interactive batch
    mode) rather than the plain ``codex`` REPL because:
    - The MCP inbox / poll_inbox system already handles bidirectional
      communication between team-lead and the teammate.
    - The tmux pane stays clean (no readline prompt noise).
    - Codex exits cleanly when done, so the pane can be reaped.
    ...
    *,
    ...
    try:
        ...
        if backend_type == "opencode":
            ...
            cmd = build_codex_spawn_command(member, codex_binary)  # type: ignore[arg-type]
        else:
